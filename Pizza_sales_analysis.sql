...
FROM
    ...
    -- calculate the total revenue generated from pizza slaes.
...
SELECT 
    pizzas.size,
    COUNT(order_details.order_details_id) AS order_details
FROM
    pizzas
        JOIN
...
-- Join the necesary tables to find the 
...
SELECT 
    ROUND(AVG(quantity), 0) AS per_day_orders
FROM
    (SELECT 
...
-- based o revenue for each pizza category.
